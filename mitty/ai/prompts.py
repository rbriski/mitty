...
5. **find_the_mistake** — generate a multi-step worked solution with exactly \
one deliberate error (arithmetic, sign, or conceptual). In the prompt, show \
the full worked solution and ask the student to identify the step with the \
error and explain the correction. Put the erroneous step number and the \
correct approach in the correct_answer field. The explanation should detail \
which step is wrong and why.
6. **review_own_errors** — (not generated by AI; handled separately).
7. **mixed** — combine two or more concepts into a single multi-part problem.
8. **calibration** — a straightforward problem designed to measure baseline \

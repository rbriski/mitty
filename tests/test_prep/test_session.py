...
class TestConfidenceVsPerformance:
    ...
    def test_comparison_with_data(self) -> None:
        engine = _make_engine()
        concept = _CONCEPTS[0]
        engine.record_confidence(concept=concept, rating=5)  # high confidence
        engine.record_answer(correct=True, concept=concept)
        engine.record_answer(correct=False, concept=concept)
        # mastery = 0.5, avg confidence = 5/5 = 1.0 -> overconfident
    ...
    def test_comparison_calibrated(self) -> None:
        engine = _make_engine()
        concept = _CONCEPTS[0]
        engine.record_confidence(concept=concept, rating=3)  # 3/5 = 0.6
        # 3 correct out of 5 = 0.6 mastery
        for _ in range(3):
            engine.record_answer(correct=True, concept=concept)
        for _ in range(2):
            engine.record_answer(correct=False, concept=concept)
    ...
    def test_comparison_underconfident(self) -> None:
        engine = _make_engine()
        concept = _CONCEPTS[0]
        engine.record_confidence(concept=concept, rating=1)  # 1/5 = 0.2
        # 4 correct out of 5 = 0.8 mastery
        for _ in range(4):
            engine.record_answer(correct=True, concept=concept)
        engine.record_answer(correct=False, concept=concept)
        rows = engine.get_confidence_vs_performance()
